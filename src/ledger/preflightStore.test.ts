...
insertPreflightRun({ id: "pf-test-1", started_at: new Date(T0).toISOString(), status: "running", trigger: "manual", surface: "ui" });
insertPreflightCheck({ run_id: "pf-test-1", seq: 1, name: "env", status: "pass", detail: '{"missing_keys":[],"key_file_readable":true}', duration_ms: 3 });
...
insertPreflightRun({ id: "pf-test-run-a", started_at: new Date(T0 + 2000).toISOString(), status: "running", trigger: "manual", surface: "ui" });
let conflict = false;
try {
  insertPreflightRun({ id: "pf-test-run-b", started_at: new Date(T0 + 3000).toISOString(), status: "running", trigger: "manual", surface: "ui" });
} catch (e) {
...
insertPreflightRun({ id: "pf-test-pass", started_at: new Date(passAt).toISOString(), status: "running", trigger: "manual", surface: "ui" });
...
assert(!computeGateState(passAt + GATE_TTL_MS + 1).open, "gate: passed >24h ago → closed (soft expiry)");
{
  const g = computeGateState(passAt + GATE_TTL_MS + 60 * 60 * 1000);
  assert(!g.open && /soft 24h expiry/.test(g.reason), `gate: expiry reason mentions soft expiry (got "${g.reason}")`);
}
...
insertPreflightRun({ id: "pf-test-repass", started_at: new Date(repassAt).toISOString(), status: "running", trigger: "manual", surface: "ui" });
...
db.prepare("DELETE FROM preflight_runs").run();
insertPreflightRun({ id: "pf-test-p2", started_at: new Date(passAt).toISOString(), status: "running", trigger: "manual", surface: "ui" });
