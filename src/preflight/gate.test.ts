...
assert(classifyAuthFailure(403, '{"error":{"message":"Insufficient client scope"}}') === "scope", "classify: 403 insufficient scope → scope");
assert(classifyAuthFailure(403, "permission denied") === "scope", "classify: 403 permission → scope");
...
triggers.length = 0;
stubFetch([{ status: 401, body: "unauthorized" }, { status: 401, body: "still unauthorized" }]);
{
  ...
  assert(triggers.length === 1 && triggers[0] === "auto-401", `http: persistent 401 → sink auto-401 (got ${triggers.join(",")})`);
}

// (2) 401 recovered by refresh (retry → 200) → NO invalidation
triggers.length = 0;
stubFetch([{ status: 401, body: "unauthorized" }, { status: 200, body: "{}" }]);
{
  const r = await httpRequest({
    method: "GET",
    url: "https://x.test/b",
  ...
  assert(triggers.length === 0, `http: recovered 401 → no invalidation (got ${triggers.join(",")})`);
}
...
{
  await httpRequest({ method: "GET", url: "https://x.test/c", reportAuthFailure: true, maxRetries: 0 });
  assert(triggers.length === 1 && triggers[0] === "auto-403-scope", `http: scope 403 → auto-403-scope (got ${triggers.join(",")})`);
}
...
{
  await httpRequest({ method: "GET", url: "https://x.test/d", reportAuthFailure: true, maxRetries: 0 });
  assert(triggers.length === 0, `http: rate-limit 403 → no invalidation (got ${triggers.join(",")})`);
}
...
{
  try {
    await httpRequest({ method: "GET", url: "https://x.test/f", reportAuthFailure: true, maxRetries: 0 });
