...
const MISSING_BODY = JSON.stringify({
  errors: [{ id: "Y", title: "Resource Not Found", detail: "id does not match an existing resource", status: "404", code: "40400" }],
});

// Empty-relationship 404 → absorbed (true).
assert(isEmptyRelationship404(new HttpError(404, EMPTY_BODY, "https://api.music.apple.com/v1/me/library/playlists/p.X/tracks")), "empty-playlist 404 (code 40403) → true");
assert(
  isEmptyRelationship404(new HttpError(404, JSON.stringify({ errors: [{ title: "No Related Resources" }] }), "u")),
  "empty-playlist 404 (title 'No related resources', case-insensitive) → true",
);

// Genuinely-missing playlist 404 → must propagate (false).
assert(!isEmptyRelationship404(new HttpError(404, MISSING_BODY, "u")), "missing playlist 404 (code 40400) → false (propagates)");

// Non-404 errors → false.
assert(!isEmptyRelationship404(new HttpError(401, EMPTY_BODY, "u")), "401 with 40403-ish body → false (not a 404)");
