...
assert(
  spotifyProvider.capabilities.supportsLikedRemoval === false,
  "spotify: supportsLikedRemoval is false (additive-only)",
);
assert(
  appleProvider.capabilities.supportsLikedRemoval === false,
  "apple: supportsLikedRemoval is false (additive-only)",
);
assert(spotifyProvider.capabilities.likedReadable === true, "spotify: liked set is readable");
assert(
  appleProvider.capabilities.likedReadable === false,
  "apple: favorites NOT readable (→ empty-D idempotency)",
);
...
assert(
  (await spotifyProvider.readDestinationTracks(OWNER, { kind: "create", name: "x" })).length === 0,
  "spotify readDest: create → []",
);
assert(
  (await appleProvider.readDestinationTracks(OWNER, { kind: "create", name: "x" })).length === 0,
  "apple readDest: create → []",
);
assert(
  (await appleProvider.readDestinationTracks(OWNER, { kind: "favorites" })).length === 0,
  "apple readDest: favorites → [] (not readable, additive-safe)",
);
...
assert(
  await throwsMissingPlaylist(() =>
    spotifyProvider.writeTracks(OWNER, { kind: "playlist", id: "x" }, undefined, ["a"]),
  ),
  "spotify writeTracks: playlist target w/o playlistId throws",
);
assert(
  await throwsMissingPlaylist(() =>
    appleProvider.writeTracks(OWNER, { kind: "playlist", id: "x" }, undefined, ["a"]),
  ),
  "apple writeTracks: playlist target w/o playlistId throws",
);
...
assert(
  await throwsInvalidSource(
    () => spotifyProvider.readSourceTracks(OWNER, { kind: "favorites" }),
    "invalid_spotify_source_target",
  ),
  "spotify readSource: favorites kind → invalid target throw",
);
assert(
  await throwsInvalidSource(
    () => appleProvider.readSourceTracks(OWNER, { kind: "liked" }),
    "invalid_apple_source_target",
  ),
  "apple readSource: liked kind → invalid target throw",
);
...
const libWithCatalog: AppleLibrarySong = {
  ...
  relationships: {
    catalog: { data: [{ id: "cat-1", type: "songs", attributes: { isrc: "USUG12604763" } }] },
  },
};
{
  ...
  assert(
    c.isrc === "USUG12604763",
    "appleLibraryToCanonical: ISRC from embedded catalog relationship",
  );
  assert(
    c.title === "Song" && c.primaryArtist === "Artist" && c.sourceId === "lib-1",
    "appleLibraryToCanonical: title/artist/sourceId",
  );
...
}
{
  ...
  const libNoIsrc: AppleLibrarySong = {
    id: "lib-2",
    type: "library-songs",
    attributes: { name: "S", artistName: "A" },
  };
  assert(
    appleLibraryToCanonical(libNoIsrc).isrc === undefined,
    "appleLibraryToCanonical: no catalog + no attr isrc → undefined",
  );
  assert(
    appleLibraryToCanonical(libNoIsrc).explicit === undefined,
    "appleLibraryToCanonical: no contentRating → explicit undefined",
  );
}
{
  // Falls back to attributes.isrc when the catalog relationship is absent.
  const libAttrIsrc: AppleLibrarySong = {
    id: "lib-3",
    type: "library-songs",
    attributes: { name: "S", artistName: "A", isrc: "us-ug1-26-04763" },
  };
  assert(
    appleLibraryToCanonical(libAttrIsrc).isrc === "USUG12604763",
    "appleLibraryToCanonical: falls back to (normalized) attributes.isrc",
  );
}
