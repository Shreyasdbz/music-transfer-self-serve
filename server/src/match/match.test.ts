...
function fakeDest(id: string, opts: FakeOpts = {}): MusicProvider {
  ...
  return {
    ...
    searchByTerm: async (
      ...
      _term: string,
      _limit: number,
    ): Promise<CanonicalTrack[]> => (opts.searchByTerm ? opts.searchByTerm() : []),
    readSourceTracks: async (_c: UserCtx, _t: ResolvedTarget): Promise<CanonicalTrack[]> =>
      unused(),
    readDestinationTracks: async (_c: UserCtx, _t: ResolvedTarget): Promise<DestTrack[]> =>
      unused(),
...
assert(
  normalize("Don't  Stop, Believin'!") === "don t stop believin",
...
assert(
  normTitle("Day 'N' Nite (Nightmare)") === "day n nite nightmare",
  "normTitle: punctuation collapse",
);
...
assert(
  variantTokens("Track (Live at Madison Square Garden)").includes("live"),
  "variantTokens: catches Live",
);
assert(variantTokens("Plain title").length === 0, "variantTokens: clean title returns []");

const isrcTrack: CanonicalTrack = {
  isrc: "USUG12604763",
  title: "x",
  primaryArtist: "y",
  artists: ["y"],
...
const noIsrcTrack: CanonicalTrack = {
  isrc: undefined,
  title: "Dust (feat. Drake)",
  primaryArtist: "Drake",
  artists: ["Drake"],
  album: undefined,
  durationMs: 180_000,
...
const wrongArtist = score(explicit, {
  ...explicit,
...
assert(
  wrongArtist.total === 70 && wrongArtist.primaryArtist === 0,
  `score: wrong artist drops 30 (got ${wrongArtist.total})`,
);
assert(isAccepted(wrongArtist), "isAccepted: 70 exactly clears");

const wrongDuration = score(explicit, {
  ...explicit,
  durationMs: 600_000,
  source: "apple",
  sourceId: "a3",
...
assert(
  wrongDuration.total === 85 && wrongDuration.duration === 0,
  `score: wrong duration drops 15 (got ${wrongDuration.total})`,
);

const remix = score(explicit, {
  ...explicit,
...
const explicitMismatch = score(explicit, {
  ...explicit,
  explicit: false,
  source: "apple",
  sourceId: "a5",
...
const noFeaturedDrift = score(
  { ...explicit, title: "Senorita" },
  {
    isrc: undefined,
    title: "Senorita (feat. Camila)",
    primaryArtist: "Eminem",
    artists: ["Eminem"],
...
assert(
  noFeaturedDrift.title === 40,
  `score: (feat. ...) stripped before matching (got title=${noFeaturedDrift.title})`,
);
...
assert(
  cacheResult.tier === "isrc" && cacheResult.confidence === 100,
  "matcher: cache returns persisted tier+confidence",
);
assert(
  cacheResult.destination?.id === "apple-cached-id",
  `matcher: cache returns persisted apple id (got ${cacheResult.destination?.id})`,
);
...
assert(
  spc.primaryArtist === "A" && spc.artists.length === 2,
  "spotifyToCanonical: primary + all artists",
);
...
assert(
  apc.isrc === "USUG12604763" && apc.explicit === true,
  "appleCatalogToCanonical: ISRC + explicit",
);
...
assert(
  apcNoRating.explicit === undefined,
  `appleCatalogToCanonical: no contentRating → explicit undefined (got ${apcNoRating.explicit})`,
);
...
assert(
  sc.explicit === 0,
  `score: explicit=false vs unknown rating → 0 explicit points (got ${sc.explicit})`,
);

// ── isValidatedCandidate (§7 Tier-1) ────────────────────────────────────

const validC: CanonicalTrack = {
  isrc: "USUG12604763",
  title: "T",
  primaryArtist: "A",
  artists: ["A"],
...
assert(
  !isValidatedCandidate({ ...validC, primaryArtist: "  " }),
  "isValidatedCandidate: blank artist fails",
);
assert(
  !isValidatedCandidate({ ...validC, durationMs: 0 }),
  "isValidatedCandidate: zero duration fails",
);
assert(
  !isValidatedCandidate({ ...validC, durationMs: undefined }),
  "isValidatedCandidate: undefined duration fails",
);
...
}
{
  ...
  assert(
    pick?.sourceId === "c2",
    `disambiguate: non-compilation preferred over Greatest Hits (got ${pick?.sourceId})`,
  );
}
{
  const src = mk("src", { source: "spotify", album: "Nonexistent" });
...
{
  assert(
    disambiguateIsrcCandidates(srcWithAlbum, []) === undefined,
    "disambiguate: empty → undefined",
  );
  const invalid = [mk("bad", { title: "", durationMs: 0 })];
  assert(
    disambiguateIsrcCandidates(srcWithAlbum, invalid) === undefined,
    "disambiguate: all-invalid → undefined",
  );
}
...
const appleCand = (id: string, over: Record<string, unknown> = {}): CanonicalTrack =>
  appleCatalogToCanonical(fakeAppleSong(id, over));
const spotifyCand = (id: string, over: Record<string, unknown> = {}): CanonicalTrack =>
  spotifyToCanonical(fakeSpotifyTrack(id, over));

const liveSource: CanonicalTrack = {
  isrc: "USUG10000001",
  title: "Real Song",
  primaryArtist: "Real Artist",
  artists: ["Real Artist"],
  album: "Real Album",
  durationMs: 200_000,
  explicit: true,
...
// Tier-1 happy path (Spotify→Apple)
{
  ...
  assert(
    r.tier === "isrc" && r.destination?.id === "apple-real-1" && r.confidence === 100,
    `matcher Tier-1 (fake): isrc match → apple-real-1 (got tier=${r.tier} id=${r.destination?.id})`,
  );
}
...
// Tier-1 returns only an invalid stub → falls through to Tier-2 scored search
{
  ...
  const r = await matchToDestination(liveSource, dest, { useCache: false });
  assert(
    r.tier === "search" && r.destination?.id === "apple-real-2",
    `matcher: invalid Tier-1 stub → Tier-2 (got tier=${r.tier} id=${r.destination?.id})`,
  );
}
...
// Tier-2 below threshold → unmatched with rejected candidate surfaced
{
  ...
  assert(
    r.rejected?.canonical.sourceId === "apple-wrong",
    "matcher: unmatched surfaces rejected candidate",
  );
...
{
  const appleSource: CanonicalTrack = {
    isrc: "USUG10000001",
    title: "Real Song",
    primaryArtist: "Real Artist",
    artists: ["Real Artist"],
  ...
  assert(
    r.tier === "isrc" && r.destination?.id === "spotify-real-1",
    `matcher symmetric Apple→Spotify Tier-1 (got tier=${r.tier} id=${r.destination?.id})`,
  );
}
...
// and matches via Tier-2 only — even when the source HAS an ISRC.
{
  ...
  const r = await matchToDestination({ ...liveSource, sourceId: "yt-src-1" }, noIsrcDest, {
    useCache: false,
  });
  assert(!isrcCalled, "Tier-2-only provider: searchByIsrc never called when supportsIsrc:false");
  assert(
    r.tier === "search" && r.destination?.id === "yt-real",
    `Tier-2-only provider: matched via scored search (got tier=${r.tier} id=${r.destination?.id})`,
  );
}
...
// Unmatched results are NOT persisted — re-run re-queries
{
  ...
  const noMatch: CanonicalTrack = {
    isrc: undefined,
    title: "Ghost Track XYZ",
  ...
  assert(
    appleCallCount === 2,
    `matcher: unmatched re-queried live both times (got ${appleCallCount} calls)`,
  );
}
...
// wrong destination).
{
  const collidingSource: CanonicalTrack = {
    isrc: undefined,
    title: "Intro",
    primaryArtist: "Band A",
    artists: ["Band A"],
    album: undefined,
    durationMs: 60_000,
    explicit: undefined,
  ...
  assert(
    !r.fromCache,
    "fuzzy guard: mismatched norm fields → cache miss, not the wrong destination",
  );
}
...
// the lexicographically-lower (isrc, id). ───────────────────────────────
{
  const dest = fakeDest("apple", {
    searchByTerm: async () => [
      appleCand("zzz-id", { isrc: undefined }),
      appleCand("aaa-id", { isrc: undefined }),
    ],
  });
  const src: CanonicalTrack = {
    isrc: undefined,
    title: "Real Song",
    primaryArtist: "Real Artist",
    artists: ["Real Artist"],
  ...
  assert(
    r1.destination?.id === "aaa-id",
    `tie-break: lower id wins deterministically (got ${r1.destination?.id})`,
  );
...
// into a reverse-direction cache hit (v1's bidirectional caching).
{
  ...
  const spSource: CanonicalTrack = {
    isrc,
    title: "Round Trip",
    primaryArtist: "RT Artist",
    artists: ["RT Artist"],
    album: "RT Album",
    durationMs: 205_000,
    explicit: true,
    source: "spotify",
    sourceId: "rt-spotify-id",
  };
  const appleDest = fakeDest("apple", {
    searchByIsrc: async () => [appleCand("rt-apple-id", { isrc })],
  });
  const fwd = await matchToDestination(spSource, appleDest, { useCache: false });
  assert(
    fwd.tier === "isrc" && fwd.destination?.id === "rt-apple-id",
    `round-trip: forward spotify→apple persists (got ${fwd.destination?.id})`,
  );

  const apSource: CanonicalTrack = {
    isrc,
    title: "Round Trip",
    primaryArtist: "RT Artist",
    artists: ["RT Artist"],
    ...
    explicit: true,
    source: "apple",
    sourceId: "rt-apple-library-id",
  };
  const spotifyDestThrows = fakeDest("spotify", {
    searchByIsrc: async () => {
      throw new Error("round_trip_should_not_search");
    },
    searchByTerm: async () => {
      throw new Error("round_trip_should_not_search");
    },
  ...
  assert(
    rev.destination?.id === "rt-spotify-id",
    `round-trip: reverse returns the original spotify id (got ${rev.destination?.id})`,
  );
}
