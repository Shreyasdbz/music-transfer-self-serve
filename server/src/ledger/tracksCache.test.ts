...
assert(
  got?.isrc === "TESTCACHE0001" && got?.norm_title === "test title",
...
assert(
  getProviderRef(base.identity_key, "apple") === "ap-1",
  "provider ref: apple coexists with spotify",
);
assert(
  getProviderRef(base.identity_key, "youtube") === undefined,
  "provider ref: missing provider → undefined",
);
...
assert(
  getProviderRef(base.identity_key, "spotify") === "sp-1",
  "provider ref: upsert leaves other provider intact",
);
...
assert(
  getProviderRef(base.identity_key, "apple") === "ap-2",
  "provider_kind: default id unchanged",
);
assert(
  getProviderRef(base.identity_key, "apple", "library") === "lib-1",
  "provider_kind: library id separate",
);
...
assert(
  updated?.confidence === 73 && updated?.match_tier === "search",
  "overwrite: confidence + tier replaced",
);
assert(updated?.norm_title === "new title", "overwrite: norm_title replaced");
assert(
  getProviderRef(base.identity_key, "spotify") === "sp-1",
  "overwrite: provider refs survive identity re-put",
);
...
assert(
  getProviderRef(base.identity_key, "spotify") === undefined,
  "delete: provider refs cascaded away",
);
assert(
  getProviderRef(base.identity_key, "apple") === undefined,
  "delete: all provider refs cascaded",
);
