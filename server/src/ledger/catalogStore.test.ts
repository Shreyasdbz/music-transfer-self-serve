...
assert(normalizeName("  My   Playlist  ") === "my playlist", "normalizeName: trim+collapse+casefold");
assert(normalizeName("WORKOUT") === "workout", "normalizeName: casefold");

// ── upsert + read-back ───────────────────────────────────────────────────
upsertCatalogRow({ platform: "spotify", kind: "playlist", external_id: "sp1", name: "Workout", owner: "me", track_count: 12, url: "u", fetched_at: T1 });
upsertCatalogRow({ platform: "spotify", kind: "liked", external_id: LIKED_SENTINEL, name: "Liked Songs", owner: null, track_count: 1623, url: null, fetched_at: T1 });
upsertCatalogRow({ platform: "apple", kind: "favorites", external_id: FAVORITES_SENTINEL, name: "Favorite Songs", owner: null, track_count: null, url: null, fetched_at: T1 });
...
assert(sp.some((r) => r.kind === "liked" && r.external_id === LIKED_SENTINEL), "sentinel: liked row present");

// upsert overwrites mutable fields
upsertCatalogRow({ platform: "spotify", kind: "playlist", external_id: "sp1", name: "Workout", owner: "me", track_count: 15, url: "u", fetched_at: T1 });
assert(getCatalogByPlatform("spotify").find((r) => r.external_id === "sp1")?.track_count === 15, "upsert: track_count updated");

assert(lastFetchedAt("spotify") === T1, "lastFetchedAt");

// ── findCatalogByName (§9 disambiguation source) ─────────────────────────
upsertCatalogRow({ platform: "spotify", kind: "playlist", external_id: "sp2", name: "workout", owner: "me", track_count: 5, url: null, fetched_at: T1 }); // dup name, different case
const matches = findCatalogByName("spotify", "  WORKOUT ");
assert(matches.length === 2, `findCatalogByName: case/space-insensitive match → 2 (got ${matches.length})`);
assert(findCatalogByName("spotify", "nonexistent").length === 0, "findCatalogByName: no match → 0");
assert(findCatalogByName("spotify", "Liked Songs").length === 0, "findCatalogByName: ignores liked/favorites rows");

// ── stale removal ────────────────────────────────────────────────────────
const T2 = "2026-06-04T11:00:00.000Z";
upsertCatalogRow({ platform: "spotify", kind: "playlist", external_id: "sp1", name: "Workout", owner: "me", track_count: 15, url: "u", fetched_at: T2 }); // refreshed
// sp2 + liked still at T1 → stale relative to T2
deleteStaleCatalogRows("spotify", T2);
const afterStale = getCatalogByPlatform("spotify");
assert(afterStale.length === 1 && afterStale[0]?.external_id === "sp1", `stale removal: only T2 row kept (got ${afterStale.map((r) => r.external_id).join(",")})`);
// apple untouched
