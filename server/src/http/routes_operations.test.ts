...
assert(
  parseUrl("spotify", "https://open.spotify.com/playlist/37i9dQZF1DXcBWIGoYBM5M") ===
    "37i9dQZF1DXcBWIGoYBM5M",
  "parseUrl: spotify https URL",
);
assert(
  parseUrl("spotify", "spotify:playlist:37i9dQZF1DXcBWIGoYBM5M") === "37i9dQZF1DXcBWIGoYBM5M",
  "parseUrl: spotify URI",
);
assert(parseUrl("spotify", "My Playlist") === undefined, "parseUrl: a name is not a URL");
assert(
  parseUrl("spotify", "37i9dQZF1DXcBWIGoYBM5M") === undefined,
  "parseUrl: a bare id is NOT a URL (must go through name resolution first)",
);
assert(
  parseUrl("apple", "https://music.apple.com/us/playlist/chill/pl.abc123") === "pl.abc123",
  "parseUrl: apple URL",
);
...
assert(
  !looksLikeBareId("spotify", "My 22 Character Playlist!"),
  "bareId: a name with spaces/punct is not an id",
);
