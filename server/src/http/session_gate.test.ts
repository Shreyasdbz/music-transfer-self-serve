...
try {
  ...
  assert(noCookie.status === 401, `protected /api/auth/status without cookie → 401 (got ${noCookie.status})`);
  assert(((await noCookie.json()) as { error?: string }).error === "unauthenticated", "401 body {error:'unauthenticated'}");
  ...
  assert(sessBody.required === true && sessBody.authenticated === false, "GET /api/session → {required:true, authenticated:false} pre-login");
  ...
  assert(((await wrong.json()) as { error?: string }).error === "invalid_access_token", "wrong token → {error:'invalid_access_token'}");
  ...
  assert(setCookie.startsWith("mtss_session="), `login sets mtss_session cookie (got "${setCookie.slice(0, 40)}")`);
  assert(/HttpOnly/i.test(setCookie) && /SameSite=Strict/i.test(setCookie), "cookie is HttpOnly + SameSite=Strict");
  ...
  assert(withCookie.status === 200, `protected /api/auth/status WITH cookie → 200 (got ${withCookie.status})`);
  ...
  assert(/Max-Age=0/i.test(cleared), `logout clears cookie (Max-Age=0) (got "${cleared.slice(0, 60)}")`);
